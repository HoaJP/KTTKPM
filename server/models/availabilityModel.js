...
//   static deleteAssignment(teacherId, classId, callback) {
//     const sql = `
//       DELETE FROM teaching_assignments
//       WHERE teacher_id = ? AND class_id = ?
//     `;
//     db.run(sql, [teacherId, classId], function (err) {
//       if (err) {
//         return callback(err, null);
//       }
//       if (this.changes === 0) {
//         return callback(new Error("Không tìm thấy bản ghi đăng ký"), null);
//       }
//       return callback(null, { teacher_id: teacherId, class_id: classId });
//     });
//   }
// }
...
class AvailabilityModel {
  ...
  static getClassById(id, callback) {
    ...
    const scheduleSql = `
      ...
      ORDER BY date
    `;
    db.get(classSql, [id], (err, classData) => {
      if (err) {
        return callback(err, null);
      }
      if (!classData) {
      ...
      db.all(scheduleSql, [id], (err, schedules) => {
        ...
        return callback(null, { ...classData, schedules });
      });
    });
  }
